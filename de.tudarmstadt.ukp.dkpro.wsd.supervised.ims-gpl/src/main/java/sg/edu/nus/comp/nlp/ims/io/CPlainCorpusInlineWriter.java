...
/**
 * result writer for plain corpus with inline disambiguations
 * @author zesch
 */
public class CPlainCorpusInlineWriter implements IResultWriter {
	// save directory
	 ...
	 * default constructor
	 */
	public CPlainCorpusInlineWriter() {
	 ...
	 */
	public CPlainCorpusInlineWriter(String p_SavePath) {
	 ...
	 */
	public CPlainCorpusInlineWriter(ICorpus p_Corpus) {
	 ...
	 *            save directory
	 */
	public CPlainCorpusInlineWriter(ICorpus p_Corpus, String p_SavePath) {
		this.m_Corpus = p_Corpus;
							...
							for (int l = 0; l < totals.size(); l++) {
								if (senses.get(l).size() > 0) {
									...
									for (int c = 0; c < senses.get(l).size(); c++) {
									    if (probs.get(l).get(c) > maxProb) {
									        String sense = senses.get(l).get(c);
									        maxProbSense = sense.substring(sense.indexOf('%')+1, sense.length());
									    }
									}
                                    //builder.append("%");
                                    builder.append(maxProbSense.replaceAll(":", "x"));
								}
							}
						}
					}
